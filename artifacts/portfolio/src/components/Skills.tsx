...
function ProgressBar({ level }: { level: number }) {
  ...
  return (
    <div ref={ref} className="h-1.5 rounded-full overflow-hidden" style={{ backgroundColor: "#21262d" }}>
      <motion.div
...
export default function Skills() {
  ...
  return (
    <section id="skills" className="py-28 relative" style={{ backgroundColor: "#0a0a12" }}>
      <div
        className="absolute inset-0 pointer-events-none"
        style={{ background: "radial-gradient(ellipse 50% 40% at 20% 60%, rgba(167,139,250,0.05) 0%, transparent 70%)" }}
      />
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="flex justify-center mb-10">
          <div className="relative flex p-1 rounded-xl" style={{ backgroundColor: "#0d1117", border: "1px solid #21262d" }}>
            {tabs.map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className="relative px-5 py-2 text-sm font-medium rounded-lg transition-colors duration-200 z-10 cursor-pointer"
                style={{ color: activeTab === tab ? "#000" : "#8b949e" }}
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-semibold text-sm" style={{ color: "#e6edf3" }}>{skill.name}</span>
                  </div>
                  <span
                ...
                <div className="flex justify-end mt-1">
                  <span className="text-xs" style={{ color: "#8b949e" }}>{skill.level}%</span>
                </div>
              </motion.div>
            ))}
          </motion.div>
        ...
        >
          <p className="text-sm mb-4" style={{ color: "#8b949e" }}>Also exploring</p>
          <div className="flex flex-wrap justify-center gap-3">
            {exploringTags.map((tag) => (
              <span
                key={tag}
                className="px-4 py-2 rounded-full text-sm border"
                style={{ borderColor: "rgba(167,139,250,0.3)", color: "#a78bfa", backgroundColor: "rgba(167,139,250,0.06)" }}
              >
                {tag}
              </span>
            ))}
